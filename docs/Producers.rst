...
In Minerva, a producer can be attached to the ServerStream. Usually, a MinervaProtocol
...
which causes ServerStream creation, which causes MinervaProtocol creation. This
...
If type of producer is push, ServerStream must also call ``pauseProducing`` on MinervaProtocol whenever
