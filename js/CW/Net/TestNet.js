...
CW.UnitTest.TestCase.subclass(CW.Net.TestNet, 'TestResponseTextDecoderNull').methods(
	...
	function test_oneMessage(self) {
		self._append("2:hi")
		var strings = self.decoder.receivedToByte(null);
	...
	function test_completeMessagesInPieces(self) {
		self._append("2:hi");
		var strings1 = self.decoder.receivedToByte(null);
		self.assertArraysEqual(['hi'], strings1);

		self._append("3:hey");
		var strings2 = self.decoder.receivedToByte(null);
	...
	function test_incompleteLength(self) {
		...
		self._append(":");
		var strings3 = self.decoder.receivedToByte(null);
		...
		var strings4 = self.decoder.receivedToByte(null);
		self.assertArraysEqual(['helloworld'], strings4);

		// Now adding a colon should result in a ParseError
		self._append(":");
		self.assertThrows(CW.Net.ParseError, function(){self.decoder.receivedToByte(null)});
	},
